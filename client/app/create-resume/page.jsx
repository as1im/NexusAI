...
import { Cpu, LogOut, History, User, Sparkles, ArrowLeft } from 'lucide-react';
...
export default function CreateResumePage() {
  ...
  // Fetch user profile to pre-populate Github URL if it exists
  useEffect(() => {
    ...
    fetch('http://localhost:5000/api/profile', { headers })
      .then(res => res.json())
      .then(resData => {
        if (resData.success && resData.data?.profile?.github) {
          setDefaultGithub(resData.data.profile.github);
        }
      })
      .catch(err => console.warn('[Profile Pre-fetch Failed]', err.message));
  }, [status, session]);
    ...
    <div className="min-h-screen bg-neutral-950 text-neutral-100 flex flex-col justify-between relative overflow-hidden font-sans">
      ...
      <header className="mx-auto my-4 max-w-5xl w-[92%] border border-neutral-900 bg-neutral-950/70 backdrop-blur-md py-4 px-6 rounded-2xl flex items-center justify-between sticky top-4 z-50 shadow-2xl shadow-black/80">
        ...
        <nav className="flex items-center gap-6 text-sm font-semibold text-neutral-400">
          ...
          <Link href="/create-resume" className="text-white hover:text-white transition-colors duration-200">Optimize Resume</Link>
      ...
      <section className="flex-1 max-w-7xl mx-auto w-full px-8 py-16 flex flex-col items-center justify-center relative z-10 animate-fade-in">
        {!isLoading ? (
          <>
            {/* Header / Intro */}
            <div className="text-center max-w-2xl mb-10">
              <div className="inline-flex items-center gap-1.5 bg-indigo-500/10 border border-indigo-500/20 px-3 py-1 rounded-full text-xs font-semibold text-indigo-300 mb-4">
                <Sparkles className="w-3.5 h-3.5" />
                Resume Tailoring Workspace
              </div>
              <h2 className="text-3xl font-black text-white mb-3 tracking-tight">
                Optimize Your Credentials
              </h2>
              <p className="text-neutral-400 text-sm leading-relaxed">
                Step through our AI-assisted optimization wizard. Link your GitHub repositories, upload your LinkedIn PDF, and paste your target job details.
              </p>
            </div>
            ...
            <div className="mt-8">
              <Link 
                href="/dashboard"
                className="text-xs text-neutral-500 hover:text-white transition-colors duration-200 flex items-center gap-1.5 font-semibold"
              >
      ...
      <footer className="w-full border-t border-neutral-900 py-6 px-8 flex flex-col md:flex-row items-center justify-between text-xs text-neutral-600 bg-neutral-950/40 relative z-10">
        <span>© 2026 NexusAI. Live API optimization workflow.</span>
